...
void nitrogen() {
  ...
  static const Int_t nch = 20;
  static const Int_t nrun = 5;
  ...
  for( int i = 6166; i <= 6382; i+=(6382-6166)/nch ) {
    if( ich == nch ) break;
    cout << "Chain #" << ich << " contains runs ( ";
    //for( int j = 0; j < nrun && i+j <= 6297 && ich < nch; j++ ) {
    for( int j = 0; j < nrun && i+j <= 6382 && ich < nch; j++ ) {
      cout << i+j << " ";
  ...
  for( Int_t ich = 0; ich < nch; ich++ ) {
    hist_lifetime->Add(new TH1F(Form("hist_lifetime%d",ich),";dt (ns);triplet hits",nbins,tmin,tmax));
  ...
  TObjArray* fit_ratio = new TObjArray();
  ...
  for( int ich = 0; ich < nch; ich++ ) {
	...
	for( Int_t pmt = 0; pmt < kNPMTs; pmt++ ) {
	  if( pmt_flag[pmt] ) {
	    if( pmt_peak[pmt][0] > 5 && 
		Abs(pmt_time[pmt][0]-pds_time[0]) < 30 )
		...
		(TH1F*)hist_lifetime->At(ich)->TH1F::Fill(TOF_hit);
	      if( pmt_time[pmt][j] > pds_time[0] &&
		  ( pmt_peak[pmt][0] > 5 ) &&
	...
	if( singlet != 0 || triplet != 0 )
	  (TH1F*)hist_ratio->At(ich)->TH1F::Fill(triplet/singlet);
      }
    }
    ...
    fit->Add((TH1F*)hist_lifetime->At(ich)->TH1F::Fit("fit","sr"));
    (TH1F*)hist_lifetime->At(ich)->TH1F::Draw("e");
    vector<Double_t> N2 = GetN2AndError((TFitResultPtr)fit->At(ich));
    cout << "N2: " << N2[0] << "+/-" << N2[1] << "ppm" << endl;
    
    fit_ratio->Add((TH1F*)hist_ratio->At(ich)->TH1F::Fit("gaus","s"));
  ...
  TH1F* hr  = new TH1F("hr","Triplet/singlet ratio;;Ratio", nch*2-1, timeBins);
  TH1F* htT = new TH1F("htT","Triplet lifetime;;#tau_{T} (ns)", nch*2-1, timeBins);
  TH1F* hN2 = new TH1F("hN2",";;Eq. [N_{2}] (ppm)",nch*2-1, timeBins);
  TH1F* hO2 = new TH1F("hO2",";;Eq. [O_{2}] (ppm)",nch*2-1, timeBins);
  for( Int_t ich = 0; ich < nch; ich++ ) {
    if(((TChain*)ch->At(ich))->GetEntries() == 0 ) continue;
    vector<Double_t> N2 = GetN2AndError((TFitResultPtr)fit->At(ich));
    vector<Double_t> tT = GettTAndError((TFitResultPtr)fit->At(ich));
    vector<Double_t> O2 = GetO2AndError((TFitResultPtr)fit->At(ich));
    vector<Double_t> ratio = GetRatioAndError((TFitResultPtr)fit_ratio->At(ich));
    cout << "N2: " << N2[0] << "+/-" << N2[1] << "ppm" << endl;
    cout << "Time: " << timeBins[ich*2] << ", " << time[ich] << "," << timeBins[ich*2+1] << endl;
    hN2->Fill(time[ich], N2[0]);
    hN2->SetBinError(hN2->FindBin(time[ich]),N2[1]);

    htT->Fill(time[ich], tT[0]);
    htT->SetBinError(htT->FindBin(time[ich]),tT[1]);

    hO2->Fill(time[ich], O2[0]);
    hO2->SetBinError(hO2->FindBin(time[ich]),O2[1]);

    hr->Fill(time[ich], ratio[0]);
    hr->SetBinError(hr->FindBin(time[ich]),ratio[1]);
  }
...
vector<Double_t> GetO2AndError(TFitResultPtr& f) {
  ...
  O2[0] = (1 / tau1 - 1 / tau0 - 0.000110 * 2.2) / kQ;
  O2[1] = Sqrt(Power(kQ_err * (1 / tau1 - 1 / tau0) / kQ / kQ, 2) +
               Power(tau0_err / kQ / tau0 / tau0, 2) +
               Power(tau1_err / kQ / tau1 / tau1, 2) + 
	       Power(0.2 * 0.000110 / kQ, 2) +
	       Power(0.000010 * 2.2 / kQ, 2) );
