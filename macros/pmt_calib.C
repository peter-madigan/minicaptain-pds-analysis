...
void pmt_calib() {
  ...
  for( Int_t pmt = 0; pmt < kNPMTs; pmt++ ) {
    ...
    h_peak->Add(new TH1F(name_peak,name_peak+";height;count",30,-30,-2));
  ...
  for( Int_t pmt = 0; pmt < kNPMTs; pmt++ ) {
    ...
    Double_t low_edge = -15;
    Double_t high_edge = -2;
    Double_t max = ((TH1F*)h_peak->At(pmt))->GetMaximum();
    Double_t pe_peak = TMath::Max(((TH1F*)h_peak->At(pmt))->GetBinContent(((TH1F*)h_peak->At(pmt))->FindBin(-10)),1e-4);

    fit->SetRange(low_edge, high_edge);
    fit->SetParameters(max,-3,1,pe_peak,-10,1);
    fit->SetParLimits(0,0,2);
    fit->SetParLimits(1,-4,0);
    fit->SetParLimits(2,low_edge,0);
    fit->SetParLimits(3,0,2);
    fit->SetParLimits(4,low_edge,high_edge);
    fit->SetParLimits(5,low_edge,0);

    c_peak->cd(1+pmt)->SetLogy();
    std::cout << std::endl;
    std::cout << "vvv\t" << "Fit results for " << pmt+1 << "\tvvv" << std::endl;
    std::cout << "~~~\t" << "Height #" << pmt+1 << "\t~~~" << std::endl;
    ((TH1F*)h_peak->At(pmt))->Fit("fit","r l");
    ((TH1F*)h_peak->At(pmt))->Draw("");
    c_peak->Update();
